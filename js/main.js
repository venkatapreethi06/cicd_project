...
// Weather Timeline Functionality
async function fetchWeatherTimeline(lat, lon) {
    console.log('Fetching weather timeline for:', lat, lon);
    let url;
    if (lat !== undefined && lon !== undefined) {
        // Use coordinates if provided
        url = `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=${apiKey}&units=metric`;
    } else {
        // Fallback to city name from input
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error('Failed to fetch weather timeline data');
        }

        const data = await response.json();
        console.log('Weather timeline data received:', data);
        displayWeatherTimeline(data);
    } catch (error) {
        console.error('Error fetching weather timeline:', error);
        const timelineContent = document.querySelector('.timeline-content');
        if (timelineContent) {
            timelineContent.innerHTML = '<p class="error-message">Unable to load weather timeline. Please try again later.</p>';
        }
    }
}

function displayWeatherTimeline(data) {
    console.log('Displaying weather timeline with data:', data);
    const timelineContent = document.querySelector('.timeline-content');
    if (!timelineContent) {
    ...
    const weatherTimeline = timelineContent.querySelector('#weather-timeline');
    if (!weatherTimeline) {
        console.error('Weather timeline element not found');
        return;
    }
    ...
    console.log('Next 24 hours data:', next24Hours);
    ...
    next24Hours.forEach((forecast, index) => {
        ...
        const precipitation = forecast.rain ? forecast.rain['3h'] || 0 : 0;
        ...
        // Determine if it's currently precipitating
        const isPrecipitating = totalPrecipitation > 0;

        timelineItem.innerHTML = `
            <div class="timeline-time">${hour}:00</div>
            <div class="timeline-weather">
                <img src="https://openweathermap.org/img/wn/${weather.icon}@2x.png" alt="${weather.description}" class="timeline-icon">
                <div class="timeline-weather-info">
                    <div class="timeline-temp">${temp}°C</div>
                    <div class="timeline-condition">${weather.description}</div>
                </div>
            </div>
            ${isPrecipitating ? `<div class="precipitation-badge">${totalPrecipitation.toFixed(1)}mm</div>` : ''}
        `;
...
setInterval(fetchWeatherTimeline, 30 * 60 * 1000);

// Initial load when page loads
document.addEventListener('DOMContentLoaded', function() {
    // Load weather timeline on page load
...
function fetchWeatherByLocation(lat, lon) {
    fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&units=metric&appid=${apiKey}`)
        ...
        .then(data => {
            ...
            // Update weather timeline
            console.log('Calling fetchWeatherTimeline with coordinates:', lat, lon);
            fetchWeatherTimeline(lat, lon);
        });
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    feedbackForm.addEventListener('submit', async function(e) {
        ...
        // For now, let's use a simple approach - send to a form handler or show the data
        // Since Resend has CORS issues with client-side requests, we'll use EmailJS or a different approach
